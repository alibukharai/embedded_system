...
static void i2c_test_task(void *arg)
{
    ...
    while (1) {
        //---------------------------------------------------
/*Prepare data to write on Slave by Master*/
        int size;
        ...
        if (ret == ESP_OK) {
/*read internal buffer so when master read the data is available*/
...
/*prepare the data so to overwrite the slave tx register*/
       for (i = 0; i < DATA_LENGTH; i++) {
            data[i] = i;
        }
/*overwrite the slave tx register*/
